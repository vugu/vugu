...
// Like 'All' but additionally confirm that the generated files that should have been committed are correct. Intended to be used by the GitHub Action
...
func Build() error {
	...
	err := goInstall("github.com/vugu/vugu/v2")
	if err != nil {
		return err
	}
...
// Builds and runs all of the Go unit tests for 'vugu', using the standard Go compiler.
func Test() error {
	mg.SerialDeps(Build)
	...
	for _, pkg := range packages {
		if strings.Contains(pkg, WasmTestSuiteDir) {
			continue // skip the wasm-test-suite package
		}
		err = goTest(pkg)
		if err != nil {
			return err
...
func TestVuguInitWithOpts(opts vuguGenOpts) error {
	...
	genFileName := opts.RootStructType + "_gen_js_wasm.go"
