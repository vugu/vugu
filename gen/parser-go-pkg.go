...
func (p *ParserGoPkg) Run() error {
	...
	if (!p.opts.SkipMainGo) && pkgName == "main" {
		...
		mainGoPath := filepath.Join(p.pkgPath, "main_wasm_gen.go")
		...
		if !fileExists(mainGoPath) {
			...
			t, err := template.New("_main_").Parse(`// Code generated by vugu via vugugen DO NOT EDIT.
...
`)
			...
			if err != nil {
				log.Printf("WARNING: gofmt on main_wasm_gen.go failed: %v", err)
			}
